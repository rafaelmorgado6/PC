...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        # ADD CODE HERE
        s_noise = random.gauss(motion[0], self.steering_noise)
        R = self.length/tan(motion[0] + s_noise)
        B = (motion[1] + distance_noise)/R 
        C = (self.x - R*sin(self.orientation), self.y + R*cos(self.orientation))
        Bw = self.orientation - (pi/2) + B
        new_orientation = self.orientation+B
        while new_orientation > 2*pi or new_orientation < 0:
                if new_orientation > 2*pi:
                    new_orientation -= 2*pi
                elif new_orientation < 0:
                    new_orientation += 2*pi
        self.set(C[0]+R*cos(Bw), C[1]+R*sin(Bw), new_orientation)
    ...
    def sense(self, addNoise=1): #do not change the name of this function
        Z = []
        noise = 0 if addNoise != 1 else random.gauss(0, self.bearing_noise)
        # ENTER CODE HERE
        # HINT: You will probably need to use the function atan2()


        for l in landmarks:
            v = (l[1] - self.x, l[0] - self.y)
            a = atan2(v[1],v[0]) - self.orientation
            while a > 2*pi or a < 0:
                if a > 2*pi:
                    a -= 2*pi
                elif a < 0:
...
def particle_filter(motions, measurements, N=500): # I know it's tempting, but don't change N!
    ...
    for i in range(N):
        p.append(robot())
        p[i].set_noise(bearing_noise, steering_noise, distance_noise)
    ...
    for t in range(len(motions)):
        ...
        w = []
        for r in p:
            w.append(r.measurement_prob(measurements[t]))
