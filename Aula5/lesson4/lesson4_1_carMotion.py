...
class robot:
    ...
    def move(self, motion): # motions = [[0.2, 10.] for row in range(10)]

        steering = motion[0]
        distance = motion[1]

        # Calculo do angulo da curva
        beta = (tan(steering) * distance)/self.length

        radious = distance/beta

        cx = self.x - radious*sin(self.orientation)
        cy = self.y + radious*cos(self.orientation)

        new_x = cx + radious * sin(self.orientation+beta)
        new_y = cy - radious * cos(self.orientation+beta)
