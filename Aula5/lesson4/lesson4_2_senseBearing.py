...
class robot:
    ...
    def sense(self): #do not change the name of this function
        ...
        for landmark in landmarks:
            ...
            # Recebe coordenadas das landmarks
            landmark_y, landmark_x = landmark

            # Calcular a diferença das coordenadas (cria vetor)
            delta_x = landmark_x - self.x
            delta_y = landmark_y - self.y

            # Calcular o angulo usando atan2()-> calcula arco tangente
            ang = atan2(delta_x, delta_y) - self.orientation

            # Ajustar o ângulo para estar no intervalo [0, 2π)
            ang = (ang + 2 * pi) % (2 * pi)
